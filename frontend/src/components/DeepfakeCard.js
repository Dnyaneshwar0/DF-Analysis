...
export default function DeepfakeCard({ data = {}, animationMs = 2000 }) {
  ...
  return (
    <>
      {/* -------- Verdict Card -------- */}
      <section
        className={`text-slate-100 rounded-2xl p-6 shadow-lg w-full border ${verdictBorder} ${verdictBg} transition-colors duration-500`}
      >
        ...
        <div className="bg-black/20 border border-white/10 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <span
              className={`inline-flex items-center justify-center px-6 py-2 rounded-full text-lg font-semibold tracking-wide border ${
                isFake
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            <div className="text-4xl font-semibold tabular-nums mt-1" aria-live="polite">
              {displayPct}%
            </div>
            ...
            >
              {frames.map((f, i) => {
                ...
                return (
                  ...
                  >
                    {score === maxFake && (
                      <div className="absolute top-3 left-3 bg-rose-600/90 text-white text-xs px-2 py-0.5 rounded-full">
                        Most fake
                      </div>
                    )}
      ...
      {modalIndex != null && frames[modalIndex] && (
        ...
        >
          <div
            className="relative w-[94%] max-w-[1200px] max-h-[92%] rounded-lg overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setModalIndex(null)}
              className="absolute top-3 right-3 z-10 bg-black/50 text-white rounded-full p-2 hover:bg-black/70"
              aria-label="Close"
            >
              ✕
            </button>
