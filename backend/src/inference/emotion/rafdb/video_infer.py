# src/inference/emotion/rafdb/video_infer.py
"""
Video inference script for RAF-DB model.
- Uses backend/models/emotion/rafdb/raf_model.onnx (repo-relative).
...
- Writes annotated output video to backend/data/emotion/output/.
"""
...
# ---- Prevent ONNX from probing GPUs ----
os.environ["CUDA_VISIBLE_DEVICES"] = ""
os.environ["ORT_LOG_VERBOSITY_LEVEL"] = "0"
warnings.filterwarnings("ignore", category=UserWarning, module="onnxruntime")

# ---- Repo root and paths (adjust to backend layout) ----
# file is at: backend/src/inference/emotion/rafdb/video_infer.py
...
if not ONNX_PATH.exists():
    raise FileNotFoundError(f"ONNX model not found at {ONNX_PATH}. Place raf_model.onnx in models/emotion/rafdb/")

OUT_DIR = REPO_ROOT / "data" / "emotion" / "output"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ---- ONNXRuntime session (CPU only) ----
try:
...
# ---- Core loop ----
def run_on_video(input_path: Path, output_path: Path = None) -> Path:
    ...
    out_name = output_path.name if output_path is not None else f"out_{input_path.name}"
...
if __name__ == "__main__":
    ...
    if inp is None:
        raise SystemExit("Usage: python -m src.inference.emotion.rafdb.video_infer path/to/video.mp4")

    p = Path(inp)
    if not p.exists():
        # try resolving repo-root relative (backend/data/...)
