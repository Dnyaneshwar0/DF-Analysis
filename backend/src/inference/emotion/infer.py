#!/usr/bin/env python3
# RAVDESS inference — model-only with compact JSON waveform + top-3 emotion timeseries output
...
from simple_audio_cnn import SimpleAudioCNN
...
def main(argv=None):
    if argv is None:
        argv = sys.argv[1:]
    if len(argv) < 1:
        sys.exit("Usage: python -m src.inference.emotion.ravdess.infer path/to/<name>_mel.npy")
    inp = Path(argv[0])
    if not inp.exists():
        alt = REPO_ROOT / inp
        if alt.exists():
